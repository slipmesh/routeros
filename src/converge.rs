...
//! 7-9. `routing filter rule` / `routing ospf instance` / `routing ospf area` (singletons).
...
pub async fn run(device: &MikrotikDevice, desired: &DesiredState, apply: bool) -> anyhow::Result<ConvergeReport> {
    let mut report = ConvergeReport::default();

    // 1. Pre-cleanup: stale `interface list member` rows, before any interface deletion.
    let desired_ifaces: Vec<String> = desired.wireguard_interfaces.iter().map(|i| i.name.clone()).collect();
    ...
    if apply {
        mikrotik::apply_list_members(device, &only_remove(&list_member_plan)).await?;
    }
    ...
    // 3. interface bridge (loopback, singleton) - before its own address can be set.
    let current_bridge = mikrotik::read_loopback_bridge(device, &desired.loopback_bridge.name).await?;
    ...
    apply_singleton(bridge_op, apply, async |op| mikrotik::apply_loopback_bridge(device, op).await).await?;
    ...
    if apply {
        mikrotik::apply_list_members(device, &only_add_update(&list_member_plan)).await?;
    }

    // 7-9. routing filter rule / ospf instance / ospf area (singletons).
    let current_filter_rule = mikrotik::read_ospf_filter_rule(device, &desired.ospf_filter_rule.chain).await?;
    let filter_rule_op = crate::diff::singleton(current_filter_rule, desired.ospf_filter_rule.clone());
    report.record_singleton("routing filter rule", &filter_rule_op);
    apply_singleton(filter_rule_op, apply, async |op| mikrotik::apply_ospf_filter_rule(device, op).await).await?;

    let current_ospf_instance = mikrotik::read_ospf_instance(device, &desired.ospf_instance.name).await?;
    let ospf_instance_op = crate::diff::singleton(current_ospf_instance, desired.ospf_instance.clone());
    report.record_singleton("routing ospf instance", &ospf_instance_op);
    apply_singleton(ospf_instance_op, apply, async |op| mikrotik::apply_ospf_instance(device, op).await).await?;
    ...
    apply_singleton(ospf_area_op, apply, async |op| mikrotik::apply_ospf_area(device, op).await).await?;

    // 10. routing ospf interface-template (exclusive).
    let current_templates = mikrotik::read_ospf_interface_templates(device).await?;
    let template_plan = crate::diff::ospf_interface_templates(&current_templates, &desired.ospf_interface_templates);
    ...
    // 12. routing bgp instance (singleton).
    let current_bgp_instance = mikrotik::read_bgp_instance(device, &desired.bgp_instance.name).await?;
    let bgp_instance_op = crate::diff::singleton(current_bgp_instance, desired.bgp_instance.clone());
    report.record_singleton("routing bgp instance", &bgp_instance_op);
    apply_singleton(bgp_instance_op, apply, async |op| mikrotik::apply_bgp_instance(device, op).await).await?;

    // 13. routing bgp connection (exclusive).
    let current_connections = mikrotik::read_bgp_connections(device).await?;
    let connections_plan = crate::diff::bgp_connections(&current_connections, &desired.bgp_connections);
